...
```{r}
tweets$date <- as.Date(tweets$created_at)
tweet_corpusnews <- corpus(tweets, text_field = "tweet", docvars = c("date", "newspaper"))

toks_news1 <- tokens(tweet_corpusnews, remove_punct = TRUE)

data_dictionary_LSD2015_pos_neg1 <- data_dictionary_LSDyes2015[1:2]
toks_news_lsd1 <- tokens_lookup(toks_news1, dictionary = data_dictionary_LSD2015_pos_neg1)

dfmat_news_lsd1 <- dfm(toks_news_lsd1) %>% 
...
# plot positive and negative valence over time
matplot(dfmat_news_lsd1$date, dfmat_news_lsd1, type = "l", lty = 1, col = 1:2,
        ylab = "Frequency", xlab = "")
grid()
legend("topleft", col = 1:2, legend = colnames(dfmat_news_lsd), lty = 1, bg = "white")
```
